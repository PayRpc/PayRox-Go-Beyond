...
async function stageChunks(manifest: any, artifacts: any, ethers: any) {
  ...
  for (const facet of manifest.facets) {
    ...
    // Calculate deployment address using production CREATE2 utilities
    const salt = generatePayRoxSalt(
      facet.name,
      manifest.version,
    ...
    // Estimate gas for deployment using production utilities
    const gasEstimate = estimateDeploymentGas(optimizedBytecode);
...
// Note: Old getDeploymentSalt and calculateCreate2Address functions removed
// Now using production utilities from './utils/create2'
...
async function validateChunks(stagedChunks: any[]) {
  ...
  for (const chunk of stagedChunks) {
    ...
    if (chunk.size > security.security.maxFacetSize) {
      throw new Error(
        `Chunk ${chunk.name} size (${chunk.size}) exceeds maximum (${security.security.maxFacetSize})`
      );
    }
    ...
    if (chunk.gasLimit > security.invariants.totalGasLimit) {
      throw new Error(
        `Chunk ${chunk.name} gas limit (${chunk.gasLimit}) exceeds maximum (${security.invariants.totalGasLimit})`
      );
    }
    ...
    for (const selector of chunk.selectors) {
      if (forbiddenSelectors.includes(selector)) {
        throw new Error(
          `Chunk ${chunk.name} contains forbidden selector: ${selector}`
        );
      }
    }
  ...
  for (const chunk of stagedChunks) {
    for (const selector of chunk.selectors) {
      if (selectorToChunk.has(selector)) {
        ...
        console.error(
          `   Used in: ${selectorToChunk.get(selector)} and ${chunk.name}`
        );
      }
  ...
  if (allSelectors.length !== uniqueSelectors.size) {
    console.error(
      `❌ Found ${
        allSelectors.length - uniqueSelectors.size
      } duplicate selectors`
    );
  ...
  console.log(
    `  ✓ Total size: ${stagedChunks.reduce(
...
async function saveStagingInfo(stagedChunks: any[]) {
  ...
  const stagingSummary = {
    ...
    totalSelectors: stagedChunks.reduce(
      (sum, chunk) => sum + chunk.selectors.length,
      0
    ),
