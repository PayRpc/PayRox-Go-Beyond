...
import { AISecurityBridge } from './ai-security-integration';
...
async function main() {
  ...
  try {
    ...
    if (!fs.existsSync(reportPath)) {
      ...
      if (!report.aiAnalysis?.automatedFixes || report.aiAnalysis.automatedFixes.length === 0) {
...
async function applyFixesFromFindings(fixes: any[], options: FixOptions) {
...
async function applyIndividualFix(fix: any, options: FixOptions): Promise<void> {
...
export { applyFixesFromFindings, applyIndividualFix };
