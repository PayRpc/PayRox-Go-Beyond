...
async function main() {
    ...
    try {
        ...
        const tx2 = await dispatcherAsFacetA.storeData("integration-test", testData);
    ...
    try {
        ...
        const testItems = [ethers.toUtf8Bytes("item1"), ethers.toUtf8Bytes("item2")];
        const tx5 = await dispatcherAsFacetB.batchProcess(testItems, deployer.address);
