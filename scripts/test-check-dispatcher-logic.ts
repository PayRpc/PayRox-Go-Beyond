...
async function testAddressValidation(results: TestResult[]): Promise<void> {
  ...
  results.push({
    ...
    passed: isValid,
    message: isValid ? 'Valid address correctly recognized' : 'Failed to recognize valid address'
  });
  ...
  results.push({
    ...
    passed: isInvalid,
    message: isInvalid ? 'Invalid address correctly rejected' : 'Failed to reject invalid address'
  });
}
  ...
  for (const relativePath of possiblePaths) {
    ...
    if (fs.existsSync(fullPath)) {
      try {
        ...
        if (deploymentData.address && ethers.isAddress(deploymentData.address)) {
    ...
    for (const relativePath of possiblePaths) {
      ...
      if (fs.existsSync(fullPath)) {
        ...
        if (deploymentData.address) {
          const contractCode = await ethers.provider.getCode(deploymentData.address);
...
async function testErrorHandling(results: TestResult[]): Promise<void> {
  ...
  const nonExistentPath = path.join(__dirname, '../deployments/nonexistent/dispatcher.json');
...
function displayTestResults(results: TestResult[]): void {
  ...
  if (passedCount === totalCount) {
    console.log('Check dispatcher state logic validation complete - All tests passed!');
  } else {
