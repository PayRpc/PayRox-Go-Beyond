...
async function main() {
  ...
  for (let i = 0; i < routeMappings.length; i += batchSize) {
    ...
    try {
      ...
      console.error(`  ❌ Error applying batch ${batchNum}:`, error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  for (const selector of testSelectors) {
    try {
      ...
      console.log(`❌ ${selector}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
