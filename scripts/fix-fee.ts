...
const FACTORY_ADDRESS = "0x5FbDB2315678afecb367f032d93F642f64180aa3"; // From deployment
...
async function main() {
  ...
  try {
    ...
    console.log(`\n📊 Current State:`);
    console.log(`   Fee: ${currentFee} wei (${ethers.formatEther(currentFee)} ETH)`);
    ...
    console.log(`\n🎯 Target State:`);
    console.log(`   Fee: ${CORRECT_FEE_WEI} wei (${ethers.formatEther(CORRECT_FEE_WEI)} ETH)`);
    console.log(`   Enabled: ${feesEnabled}`);
    console.log(`   Recipient: ${feeRecipient}`);
    ...
    if (!hasFeeRole) {
      ...
      if (hasAdminRole) {
        ...
        console.log("❌ Deployer doesn't have admin privileges. Cannot fix fee.");
        return;
      }
    } else {
    ...
    console.log(`   New fee: ${newFee} wei (${ethers.formatEther(newFee)} ETH)`);
