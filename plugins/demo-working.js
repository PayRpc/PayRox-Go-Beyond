...
async function runWorkingDemo() {
  try {
    ...
    if (!fs.existsSync(sdkPath)) {
      throw new Error(
        'PayRox SDK not found. Please run from the correct directory.'
      );
    }
    ...
    try {
      ...
      console.log(
        'ℹ️  Token deployment example completed (may show expected errors)'
      );
    }
...
`);
    ...
    console.log(
      '2. Ensure PayRox contracts are deployed (run deploy-complete-system.ts)'
    );
