...
describe("FacetSizeCap", function () {
  const MAX_FACET_RUNTIME_SIZE = 24576; // EIP-170 limit
  ...
  it("Should fail CI if any production facet exceeds EIP-170 limit", async function () {
    ...
    results.forEach(({ name, size }) => {
      ...
      console.log(`  ${status} ${name}: ${size} bytes (${percentage}% of EIP-170 limit)`);
    });

    expect(allWithinLimits).to.equal(true, "CRITICAL: One or more facets exceed EIP-170 limit - deployment will fail");
  });
});
